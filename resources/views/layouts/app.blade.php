...
<head>
    ...
    <style>
        ...
        .sidebar .nav-link.active { 
            background: linear-gradient(90deg, #cd2122 0%, #ff4d4d 100%); 
            color: #ffffff; 
    ...
    <div class="sidebar d-flex flex-column">
        ...
        <div class="sidebar-header">
            ...
            <div class="mt-3 badge bg-white bg-opacity-10 text-white fw-light px-3 py-2 rounded-pill border border-white border-opacity-10">
                @if(Auth::guard('admin')->check()) 
                    <i class="bi bi-shield-lock me-1"></i> {{ __('messages.admin_portal') }}
                @elseif(Auth::guard('teacher')->check()) 
                    <i class="bi bi-person-video3 me-1"></i> {{ __('messages.teacher_portal') }}
                @elseif(Auth::guard('parent')->check()) 
                    <i class="bi bi-house-heart me-1"></i> {{ __('messages.parent_portal') }}
                @endif
            </div>
        </div>
        ...
        <ul class="nav flex-column mt-4 mb-auto">
            ...
            @if(Auth::guard('admin')->check())
                ...
                <li class="nav-item"><a class="nav-link {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}" href="{{ route('admin.dashboard') }}"><i class="bi bi-grid-fill"></i> {{ __('messages.dashboard') }}</a></li>
                <li class="nav-item"><a class="nav-link {{ request()->routeIs('admin.users*') ? 'active' : '' }}" href="{{ route('admin.users') }}"><i class="bi bi-people-fill"></i> {{ __('messages.accounts') }}</a></li>
                <li class="nav-item"><a class="nav-link {{ request()->routeIs('admin.enrolment*') ? 'active' : '' }}" href="{{ route('admin.enrolment') }}"><i class="bi bi-person-vcard-fill"></i> {{ __('messages.enrolment') }}</a></li>
                <li class="nav-item"><a class="nav-link {{ request()->routeIs('admin.exams*') ? 'active' : '' }}" href="{{ route('admin.exams') }}"><i class="bi bi-journal-bookmark-fill"></i> {{ __('messages.exam_setup') }}</a></li>
                <li class="nav-item"><a class="nav-link {{ request()->routeIs('admin.finance*') ? 'active' : '' }}" href="{{ route('admin.finance') }}"><i class="bi bi-wallet-fill"></i> {{ __('messages.finance') }}</a></li>
                
                <li class="nav-item">
                     <a class="nav-link {{ request()->routeIs('admin.reports') ? 'active' : '' }}" href="{{ route('admin.reports') }}">
                        <i class="bi bi-bar-chart-fill me-2"></i>
                        <span>{{ __('messages.reports_analytics') }}</span>
                     </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('admin.reports.*') ? 'active' : '' }}" data-bs-toggle="collapse" href="#reportsMenu" role="button" aria-expanded="{{ request()->routeIs('admin.reports.*') ? 'true' : 'false' }}">
                        ...
                        <span class="flex-grow-1">{{ __('messages.pbmt_reports') }}</span>
                        ...
                        <ul class="nav flex-column ms-3 mt-1" style="border-left: 1px solid rgba(255,255,255,0.1);">
                            <li class="nav-item ps-2 mb-1">
                                <a class="nav-link py-2 {{ request()->routeIs('admin.reports.takwim') ? 'text-white fw-bold' : '' }}" href="{{ route('admin.reports.takwim') }}" style="font-size: 0.85rem;">
                                    <i class="bi bi-calendar3 fs-6"></i> {{ __('messages.takwim') }}
                                </a>
                            </li>
                            <li class="nav-item ps-2 mb-1">
                                <a class="nav-link py-2 {{ request()->routeIs('admin.reports.unjuran') ? 'text-white fw-bold' : '' }}" href="{{ route('admin.reports.unjuran') }}" style="font-size: 0.85rem;">
                                    <i class="bi bi-calculator fs-6"></i> {{ __('messages.unjuran') }}
                                </a>
                            </li>
                            <li class="nav-item ps-2 mb-1">
                                <a class="nav-link py-2 {{ request()->routeIs('admin.reports.berkelompok') ? 'text-white fw-bold' : '' }}" href="{{ route('admin.reports.berkelompok') }}" style="font-size: 0.85rem;">
                                    <i class="bi bi-collection fs-6"></i> {{ __('messages.berkelompok') }}
                                </a>
                            </li>
                            <li class="nav-item ps-2 mb-1">
                                <a class="nav-link py-2 {{ request()->routeIs('admin.reports.prestasi') ? 'text-white fw-bold' : '' }}" href="{{ route('admin.reports.prestasi') }}" style="font-size: 0.85rem;">
                                    <i class="bi bi-graph-up fs-6"></i> {{ __('messages.prestasi') }}
                                </a>
                            </li>
                        </ul>
                    </div>
                </li>
                <li class="nav-item mt-1"><a class="nav-link {{ request()->routeIs('admin.events*') ? 'active' : '' }}" href="{{ route('admin.events') }}"><i class="bi bi-calendar-check-fill"></i> {{ __('messages.events') }}</a></li>
            @endif

            @if(Auth::guard('teacher')->check())
                <li class="nav-item"><a class="nav-link {{ request()->routeIs('teacher.dashboard') ? 'active' : '' }}" href="{{ route('teacher.dashboard') }}"><i class="bi bi-grid-fill"></i> {{ __('messages.dashboard') }}</a></li>
    ...
    <a class="nav-link {{ request()->routeIs('teacher.report-cards.*') ? 'active' : '' }}" 
       href="{{ \Illuminate\Support\Facades\Route::has('teacher.report-cards.index') ? route('teacher.report-cards.index') : '#' }}">
        ...
        <span>{{ __('messages.academic_report_cards') ?? 'Report Cards' }}</span>
                ...
                <li class="nav-item"><a class="nav-link {{ request()->routeIs('teacher.communication*') ? 'active' : '' }}" href="{{ route('teacher.communication') }}"><i class="bi bi-chat-quote-fill"></i> {{ __('messages.chat') }}</a></li>
                
                <li class="nav-item">
                    <a href="{{ route('teacher.events') }}" class="nav-link {{ request()->routeIs('teacher.events') ? 'active' : '' }}">
                        <i class="bi bi-calendar-event-fill"></i> {{ __('messages.events') }}
                    </a>
                </li>
            @endif
           ...
           @if(Auth::guard('parent')->check())
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('parent.dashboard') ? 'active' : '' }}" href="{{ route('parent.dashboard') }}">
                        <i class="bi bi-grid-fill"></i> {{ __('messages.dashboard') }}
                    </a>
                </li>
                ...
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('parent.communication') ? 'active' : '' }}" href="{{ route('parent.communication') }}">
                        <i class="bi bi-chat-dots-fill"></i> {{ __('messages.chat') }}
                    </a>
                </li>

                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('parent.payment') ? 'active' : '' }}" href="{{ route('parent.payment') }}">
                        <i class="bi bi-credit-card-fill"></i> {{ __('messages.payment') }}
                    </a>
                </li>
        ...
        <div class="p-3 mt-auto">
            ...
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="btn btn-outline-danger w-100 py-2 rounded-3 d-flex align-items-center justify-content-center" style="border-color: rgba(255,255,255,0.2); color: #ffadad;">
