...
<div class="container-fluid pb-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="mb-0 fw-bold text-dark">{{ __('messages.school_events') }}</h3>
        <button class="btn btn-warning fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#addEventModal">
            <i class="bi bi-megaphone-fill"></i> {{ __('messages.post_announcement') }}
        </button>
    </div>
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm border-0 h-100">
                <div class="card-header bg-white py-3 border-0">
                    <h6 class="mb-0 fw-bold text-dark">{{ __('messages.upcoming_activities') }}</h6>
                </div>
                                ...
                                <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0">
                                    <li>
                                        <button class="dropdown-item edit-btn small" 
                                            ...
                                            <i class="bi bi-pencil-square me-2 text-muted"></i> {{ __('messages.edit') }}
                                        </button>
                                    </li>
                                    <li>
                                        <form action="{{ route('admin.events.delete', $event->event_id) }}" method="POST" onsubmit="return confirm('{{ __('messages.delete_event_confirm') }}');">
                                            @csrf @method('DELETE')
                                            <button class="dropdown-item text-danger small"><i class="bi bi-trash me-2"></i> {{ __('messages.delete') }}</button>
                                        </form>
                                    </li>
                                </ul>
                            </div>
    ...
    <div class="modal-dialog">
        <form class="modal-content border-0 shadow" action="{{ route('admin.events.store') }}" method="POST">
            ...
            <div class="modal-header bg-warning border-0">
                <h5 class="modal-title text-dark fw-bold">{{ __('messages.new_announcement') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4 bg-light">
                <div class="mb-3">
                    <label class="small fw-bold text-muted">{{ __('messages.event_title') }}</label>
                    <input type="text" name="title" class="form-control border-0 shadow-sm" required>
                </div>
                <div class="mb-3">
                    <label class="small fw-bold text-muted">{{ __('messages.description') }}</label>
                    <textarea name="description" class="form-control border-0 shadow-sm" rows="2"></textarea>
                </div>
                <div class="row">
                    <div class="col-6 mb-3">
                        <label class="small fw-bold text-muted">{{ __('messages.start_date') }}</label>
                        <input type="date" name="start_date" class="form-control border-0 shadow-sm" required>
                    </div>
                    <div class="col-6 mb-3">
                        <label class="small fw-bold text-muted">{{ __('messages.end_date_optional') }}</label>
                        <input type="date" name="end_date" class="form-control border-0 shadow-sm">
                    </div>
                </div>
                <div class="mb-3">
                    <label class="small fw-bold text-muted">{{ __('messages.type_theme') }}</label>
                    <select name="theme" class="form-select border-0 shadow-sm">
                        <option value="primary">{{ __('messages.activity_blue') }}</option>
                        <option value="danger">{{ __('messages.holiday_red') }}</option>
                    </select>
                </div>
            </div>
            <div class="modal-footer border-0 bg-light">
                <button type="submit" class="btn btn-warning w-100 fw-bold shadow-sm rounded-pill">{{ __('messages.post_event') }}</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="modal-dialog">
        <form class="modal-content border-0 shadow" id="editEventForm" method="POST">
            ...
            <div class="modal-header bg-primary text-white border-0">
                <h5 class="modal-title fw-bold">{{ __('messages.edit_event') }}</h5>
            ...
            <div class="modal-body p-4 bg-light">
                ...
                <div class="row">
                    ...
                    <div class="col-6 mb-3">
                        <label class="small fw-bold text-muted">{{ __('messages.end_date') }}</label> <input type="date" name="end_date" id="edit_end" class="form-control border-0 shadow-sm">
                    </div>
                </div>
                <div class="mb-3">
                    <label class="small fw-bold text-muted">{{ __('messages.type_theme') }}</label>
            ...
            <div class="modal-footer border-0 bg-light">
                <button type="submit" class="btn btn-primary w-100 fw-bold shadow-sm rounded-pill">{{ __('messages.update_event') }}</button>
            </div>
        </form>
    </div>
</div>
